...
describe('Coop client', () => {
    ...
    it('should make a GET request', async () => {
        const coop = new Coop();
		const client = coop.getClient();
		requestMock.mockReturnValueOnce({ statusText: 'OK', data: { message: 'Hello World' } });
    ...
    it('should error from the GET request', async () => {
        const coop = new Coop();
		const client = coop.getClient();
        requestMock.mockReturnValueOnce({ statusText: 'Internal Server Error' });
        try {
            await client.get('/');
        } catch (e) {
    ...
    it('should log the request in the console', async () => {
		...
		const client = coop.getClient();
        requestMock.mockReturnValueOnce({ statusText: 'OK' });
        await client.get('/');
